...
Deno.serve(async (req) => {
  ...
  // Client used only to authenticate/authorize the signed-in caller.
  // It intentionally keeps the caller's JWT in Authorization.
  const caller = createClient(supabaseUrl, serviceRole, {
  ...
  // IMPORTANT: do not reuse `caller` here. It has the user's JWT in the
  // Authorization header, which overrides the service-role credential for
  // the Auth Admin request and causes 403 "needs supabase_admin/service_role".
  // This client has no caller Authorization header and therefore uses the
  // service-role credential for the admin Auth API call.
  ...
  if (mode === "generate") {
    return json({
      ...
      mode: "generate",
      email,
      invitation_id: generated.user?.id || null,
  ...
  const from =
    ...
    "SaleTrening <onboarding@resend.dev>";
    ...
    return json(
      {
        ...
        message:
          "Email-сервис не подключён. Ссылка создана — используйте кнопку «Копировать».",
        invite_url: actionLink,
      },
      503,
    );
  ...
  const res = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      from,
      to: [email],
      subject: "Приглашение в SaleTrening",
      html,
    }),
  });
    ...
    return json(
      {
        ...
        message: result?.message || "Не удалось отправить письмо",
        invite_url: actionLink,
      },
      502,
    );
