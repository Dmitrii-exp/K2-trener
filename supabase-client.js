...
(function(){
  ...
  function emit(event,s){listeners.forEach(fn=>{try{fn(event,s)}catch(e){setTimeout(()=>{throw e})}})}
  function auth(){
    return {
      ...
      async updateUser(attributes){try{const d=await request('/auth/v1/user',{method:'PUT',body:JSON.stringify(attributes)},session?.access_token);if(session)save({...session,user:d});return {data:{user:d},error:null}}catch(error){return {data:{user:null},error}}},
  ...
  const client={
    ...
    auth:auth(),
    functions:{
      async invoke(name,{body,headers}={}){try{return {data:await request('/functions/v1/'+encodeURIComponent(name),{method:'POST',body:JSON.stringify(body||{}),headers:headers||{}},session?.access_token),error:null}}catch(error){return {data:null,error}}}
    },
    async rpc(name,args={}){try{return {data:await request('/rest/v1/rpc/'+encodeURIComponent(name),{method:'POST',body:JSON.stringify(args)},session?.access_token),error:null}}catch(error){return {data:null,error}}}
  };
