...
(function(){
  ...
  window.completeInviteRegistration=async function(){
    ...
    try{
      await window.acceptPendingInvitation(data?.user?.id);
      await originalBoot(data.user);
    }catch(e){console.error('completeInviteRegistration:',e);window.toast(e.message||'Не удалось принять приглашение')}
  };
  ...
  window.signUp=function(){
    window.toast('Для регистрации сотрудника используйте ссылку из приглашения на указанный Email.');
  };
  ...
  window.addEventListener('load',async function(){
    ...
    if(data?.session)window.boot(data.session.user);
      ...
      if(input&&invitedEmail){input.value=invitedEmail;input.readOnly=true;input.style.background='#f5f5f8';input.style.color='#555';input.style.cursor='not-allowed'}
    }
  });
})();
